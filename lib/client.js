...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      'word-break:break-all;max-height:300px;overflow:auto;color:var(--dsw-alias-label-primary);margin:8px 0 0}',
    ...
    function Panel() {
      ...
      async function run(label, route, method, body) {
        setBusy(true)
      ...
      function onPush() {
        ...
        setArmedPush(false)
        run('总结并推送', '/note.push', 'POST', { confirmed: true })
      }
      ...
      return h('div', { className: 'cfgsync-wrap' },
        h('div', { className: 'cfgsync-group' },
          ...
          h('div', { className: 'cfgsync-row' },
            h('button', { className: 'cfgsync-btn', disabled: busy, onClick: function () { run('导出快照', '/sync.export') } }, '导出快照'),
            h('button', { className: 'cfgsync-btn', disabled: busy, onClick: function () { run('检查漂移', '/sync.drift') } }, '检查漂移'))),
        h('div', { className: 'cfgsync-group' },
          h('p', { className: 'cfgsync-gtitle' }, '② 总结当前对话并推送'),
          h('p', { className: 'cfgsync-gdesc' }, '生成脱敏的 agent.md 摘要到私有仓。预览不推送；推送需要**显式二次确认**。'),
          h('div', { className: 'cfgsync-row' },
            h('button', { className: 'cfgsync-btn', disabled: busy, onClick: function () { run('预览摘要', '/note.preview') } }, '预览摘要'),
            h('button', {
        ...
        h('div', { className: 'cfgsync-group' },
          ...
          h('div', { className: 'cfgsync-row' },
            h('button', { className: 'cfgsync-btn', disabled: busy, onClick: function () { run('浏览汇总', '/browse.list') } }, '浏览会话汇总'))),
        h('div', { className: 'cfgsync-row' },
          h('button', { className: 'cfgsync-btn', disabled: busy, onClick: function () { run('诊断', '/diag') } }, '诊断'),
