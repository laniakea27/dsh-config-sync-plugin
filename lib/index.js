 ...
 * 也**不能写死某一个盘**（用户可能在 C:\Users\<名>\dsh-config-sync 或别处 clone）。
 *
 * 顺序 = 优先级：
 *   1. 环境变量 DSH_CONFIG_SYNC_REPO（显式指定，最可信）
 *   2. ~/dsh-config-sync（用户主目录下，最自然的位置）
 *   3. 开发机上的具体路径（历史路径，保底）
 *
...
function repoCandidates() {
  const raw = []
  const explicit = process.env.DSH_CONFIG_SYNC_REPO
  if (typeof explicit === 'string' && explicit.trim() !== '') raw.push(explicit.trim())
  try { raw.push(path.join(homedir(), 'dsh-config-sync')) } catch { /* ignore */ }
  raw.push('F:/AIagent/DeepH/dsh-config-sync')
  raw.push('F:\\AIagent\\DeepH\\dsh-config-sync')
  raw.push('/f/AIagent/DeepH/dsh-config-sync')
